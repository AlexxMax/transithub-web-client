...
<script>
import Catalog from '@/components/PQQueueProfiles/PQQueueProfilesCatalog'
...
export default {
  ...
  computed: {
    title () {
    	return this.$t('forms.common.pqQueueProfiles') + ' - Transithub'
    },
  ...
  methods: {
    ...
    handleSelect(item) {
      if (this.$_smallDeviceMixin_isDeviceSmall) {
        this.$router.push(this.$i18n.path(`workspace/pq-queue-profiles/${item.guid}`))
      } else {
